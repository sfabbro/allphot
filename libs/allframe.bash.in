...
# daophot_match <input reference star file> <output mch file> <list of star files>
daophot_match() {
    # Master input file
    echo ${1}
    # Output file name
...
daophot_master() {
    ...
    echo ${field}.mch
    echo -n "$(option_get_value MS ${2}), "
    echo -n "$(option_get_value MC ${2}), "
    echo "$(option_get_value MF ${2})"
    option_get_value MD ${2}
    option_get_value GE ${2}
    option_get_value MR ${2}
    for i in $(seq ${nframes}); do echo ; done
    for i in $(seq ${niter}); do option_get_value MI ${2}; done
    echo 0
    #  Assign new star IDs? 
    echo y
