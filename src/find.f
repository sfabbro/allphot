...
C
     ...
     .     GRPFIL*40, CONPIC*40
      CHARACTER SWITCH*40, EXTEND*40, LINE*5, CASE*4, ANSWER*1
...
C area desired is MAX (2.0, 0.72*FWHM), so the distance from the 
C central pixel to the center of an edge pixel is the integer smaller 
C than this.
C
      RADIUS=AMAX1(2.001, 0.72*FWHM)
...
C
      LY = JY+NHALF
      IF (LY .LE. NROW) THEN
         CALL RDROW (1, LY, D(1,JCYLN(NBOX)), SHOW, IER)
...
c    .        D(1,JCYLN(NBOX)), IER)
         IF (IER .NE. 0) THEN
            ...
            RETURN
         END IF
         CALL RDROW (2, LY, H(1,JCYLN(NBOX)), SHOW, IER)
c        CALL RDARAY ('COPY', LX, LY, NCOL, NROWS, MAXCOL, 
