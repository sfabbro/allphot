      SUBROUTINE  DAOPK (NCOL, NROW, PAR, MAXPAR, PSF, MAXPSF, 
     .     MAXEXP, F, WATCH, FITRAD, PERERR, PROERR)
C
C=======================================================================
C
...
C
     ...
     .     SWITCH*40
      CHARACTER LINE*80, EXTEND*40, CASE*3
...
C
      ...
      INTEGER RDPSF, NITER, IER, LX, LY, NX, NY
      INTEGER NFRAC, IBEG, ISTAR, NEXP, NPAR, NPSF, IPSTYP
      INTEGER IDUM, NL, NCOL, NROW, NTERM
C
...
C
 2000 CALL RDSTAR (2, NL, ISTAR, X, Y, APMAG, SKY)
      ...
      CALL RDARAY (1, LX, LY, NX, NY, MAXBOX, F, IER)
C
...
C
  ...
  621 FORMAT (6X, I7, ' had a singular matrix.')
      CALL STUPID (LINE)
      GO TO 2000
C
...
C
 2010 X=X+LX-1.
     ...
     .     CHI, SHARP, NITER
  622 FORMAT (6X, I7, 2F9.2, F9.3, ' +- ', F5.3, 2F7.2, I5)
      WRITE (3,320) ISTAR, X, Y, AMAG, ERRMAG, SKY, FLOAT(NITER), CHI,
     .     SHARP
  320 FORMAT (I7, 3F9.3, F9.4, F9.3, F9.0, F9.2, F9.3)
      GO TO 2000
C
C-----------------------------------------------------------------------
C
...
C        read-out noise and the photons/ADU.
C
...
C
      ...
      INTEGER I, J, IX, IY, IXLO, IXHI, IYLO, IYHI
...
C
      IF (NPIX .LT. 3) THEN
         ...
         RETURN
      END IF
      CALL INVERS (C, 3, 3, IER)
      IF (IER .EQ. 0) GO TO 2100
      NITER=-1
      RETURN
C
