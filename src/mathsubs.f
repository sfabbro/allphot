c
C This contains file subroutines that are not I/O related, but rather
...
C
      ...
      END DO
cC
cC Don't sort if already sorted
cC
c      DO I=1,N-1
c         IF (DATUM(I) .GT. DATUM(I+1)) GO TO 99
c      END DO
c      RETURN
C
...
C 
      ...
      IF (NSTAK .GT. MAXSTK) THEN
         CALL STUPID ('Stack overflow in QUICK.  Increase MAXSTK.')
         CALL OOPS
      END IF
      STKLO(NSTAK)=LIMLO
      STKHI(NSTAK)=LO-1
...
C
      ...
      REAL SIGMA, SKEW, R, SIGN, HIBAD, READNS
      REAL DMOD, OLD, CLAMP, HALF, SWT
...
C
      SKYMED=0.0
      SWT = 0.0
      HALF = REAL((MAXIMM-MINIMM-1)/2)
C
      DO 2310 I=MINIMM+1,MAXIMM
         DELTA = AMIN1(REAL(I-MINIMM-1),REAL(MAXIMM-I))/HALF
         SKYMED = SKYMED+DELTA*(SKY(I)-SKYMID)
         SWT = SWT+DELTA
 2310 CONTINUE
C
      SKYMED=SKYMED/SWT + SKYMID
...
C
  ...
  302 FORMAT (1X, A8, 4I5, F9.3, F15.3, 2F9.1)
      IF (ISTAT .NE. 0) THEN
C        CALL STUPID ('Error reading PSF.')
         CALL CLFILE (3)
         RDPSF = -1
         RETURN
      END IF
...
C
      ...
      ELSE
         Z = Y/X
         AT360 = ATAN(Z)
         IF (X .LT. 0.) AT360 = AT360+3.141 592 654
      END IF
      IF (AT360 .LT. 0.) AT360 = AT360+6.283 185 307
      RETURN
      END
C
C#######################################################################
C
      INTEGER FUNCTION  SBNRY  (ID, N, TARGET)
      CHARACTER*(*) ID(*), TARGET
      NC = LEN(TARGET)
      ILO = 0
      IHI = N+1
...
C
      III = (ILO + IHI)/2
      DO 1100 I=1,NC
         IF (ICHAR(TARGET(I:I))-ICHAR(ID(III)(I:I))) 1200,1100,1300
 1100 CONTINUE
C
C The strings match!
C
      ...
      RETURN
C
C The target is smaller than this list item.
C
 1200 IHI = III
      GO TO 1000
C
C The target is larger than this list item.
C
 1300 ILO = III
      GO TO 1000
      END
...
C#######################################################################
C
      SUBROUTINE  SQUICK (DATUM, N, INDEX)
C
C=======================================================================
C
C A quick-sorting algorithm suggested by the discussion on pages 114-119
...
C
C DATUM (INPUT/OUTPUT) is a vector of dimension N containing randomly 
C        ordered character data upon input.  Upon output the elements of 
C        DATUM will be in alphabetical order.
C
C 
C INDEX (OUTPUT) is an integer vector of dimension N.  Upon return to
C       the calling program the i-th element of INDEX will tell where
...
C
      INTEGER LEN, ICHAR
C      INTEGER LENGTH
      INTEGER I, J, K, L, HI, LO, NC, NSTAK, LIMLO, LIMHI, IKEY
      NC = LEN(DATUM(1))
C
C Initialize INDEX.
C
      DO 50 I=1,N
         INDEX(I)=I
C
C Right-justify numbers
C
C         L = LENGTH(DATUM(I))
C         DO J=L,1,-1
C            K = ICHAR(DATUM(I)(J:J))
C            IF ((K .LT. 48) .OR. (K .GT. 57)) THEN
C               IF (L .EQ. NC) GO TO 50
CC
CC We need to insert NC-L nulls
CC
C               DKEY = '!!!!!!!!!!!!!'
C               DKEY(1:J) = DATUM(I)(1:J)
C               K = NC+J-L+1
C               DKEY(K:NC) = DATUM(I)(J+1:L)
C               DATUM(I) = DKEY(1:NC)
C               GO TO 50
C            END IF
C         END DO
...
C     IF (DATUM(HI) .LE. DKEY) GO TO 109
C     HI=HI-1
C
C The pointer HI is to be left pointing at a datum SMALLER than the
C key, which is intended to be overwritten.
...
C     IF (DATUM(LO) .GE. DKEY) GO TO 119
C
C     LO=LO+1
C     GO TO 110
C
...
C
      IF (NSTAK .LE. 0) THEN
CC
CC Strip out any nulls
CC
C         DO I=1,N
C            DO J=1,NC
C 4010          IF (DATUM(I)(J:J) .EQ. '!') THEN
C                  DATUM(I)(J:NC) = DATUM(I)(J+1:NC)//' '
C                  GO TO 4010
C               END IF
C            END DO
C         END DO
...
C#######################################################################
C
      SUBROUTINE  GTFM  (X, Y, RCOL, RROW, COEFF, MODE, XX, YY)
      DOUBLE PRECISION COEFF(20)
      REAL X, Y, RCOL, RROW, XX, YY
      XX = COEFF(1) + COEFF(3)*X + COEFF(5)*Y
      YY = COEFF(2) + COEFF(4)*X + COEFF(6)*Y
      IF (MODE .GE. 7) THEN
         XS=2.*(X-1.)/(RCOL-1.)-1.
         YS=2.*(Y-1.)/(RROW-1.)-1.
         XY=XS*YS
         X2=1.5*XS**2-0.5
         Y2=1.5*YS**2-0.5
         XX=XX+COEFF(7)*X2+COEFF(9)*XY+COEFF(11)*Y2
         YY=YY+COEFF(8)*X2+COEFF(10)*XY+COEFF(12)*Y2
         IF (MODE .GE. 13) THEN
            XX = XX+
     .           (COEFF(13)*XS+COEFF(15)*YS)*X2+
     .           (COEFF(17)*XS+COEFF(19)*YS)*Y2
            YY = YY+
     .           (COEFF(14)*XS+COEFF(16)*YS)*X2+
     .           (COEFF(18)*XS+COEFF(20)*YS)*Y2
         END IF
      END IF
      RETURN
      END
...
C#######################################################################
C
      SUBROUTINE  INVERT  (COEFF, MODE, COL, ROW, COLM, ROWM, FFEOC)
...
C
      DO J=1,NPLACE
         ...
         DO I=1,NPLACE
            X = PLACES(I)*(COL-1.)+1.
            CALL GTFM (X, Y, COL, ROW, COEFF, M, XX, YY)
            D(1) = 1.
            D(2) = XX
            D(3) = YY
            XS = 2.*(XX-1.)/(COLM-1.)-1.
...
c     SUM = 0.
c     DO J=1,NPLACE
c        Y = PLACES(J)*(ROW-1.)+1.
c        DO I=1,NPLACE
c           X = PLACES(I)*(COL-1.)+1.
c           CALL GTFM (X, Y, COL, ROW, COEFF, M, XX, YY)
c           CALL GTFM (XX, YY, COLM, ROWM, FFEOC, M, XXX, YYY)
c           PRINT *, X, Y, XX, YYY, XXX-X, YYY-Y
c           SUM = SUM + (XXX-X)**2 + (YYY-Y)**2
c        END DO
c     END DO
c     PRINT *, 'Invert:', SQRT(SUM/(2.*NPLACE*NPLACE))
C
C END TEST CODE
C
      RETURN
