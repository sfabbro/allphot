...
C
     ...
     .     GRPFIL*40, SWITCH*40, EXTEND*40, CASE*4
...
C
      ...
      REAL CRITSQ, CRITSEP, DXTEST, DYTEST, RATIO, ERR
      REAL LOBAD, MTEST, NUMER, XTEST, YTEST, STEST, RONOIS
...
C
 ...
 2020 CALL RDSTAR (2, NL, ID(I), X(I), Y(I), MAG(I), SKY(I))
      ...
      IF (MAG(I) .GT. 90.) THEN
         ...
         CALL RDARAY (1, LX, LY, NX, NY, MAXBOX, DATA, IST)
         IF ((NX .LT. 1) .OR. (NY .LT. 1)) GO TO 2010
         DELTAX=(X(I)-1.)/XPSF-1.
         DELTAY=(Y(I)-1.)/YPSF-1.
         DENOM=0.
...
C
      ERR=RONOIS+0.5*(STEST+SKY(J))/PHPADU
C
...
C
 ...
 4010 IF (ISIZE(IFIRST) .NE. I) GO TO 4030
      ...
      K=INDEX(J)
      IF (SKY(K) .LT. 9999.999) THEN
         WRITE (3,340) ID(K), X(K), Y(J), MAG(K), SKY(K)
  340    FORMAT (I7, 4F9.3)
      ELSE
         WRITE (3,341) ID(K), X(K), Y(J), MAG(K), SKY(K)
  341    FORMAT (I7, 3F9.3, F9.2)
      END IF
 4020 CONTINUE
      NGRP=NGRP+1
      WRITE (3,340)
...
C
  ...
  643 FORMAT (/I7, ' star', A4, I5, ' group', A4/)
 9080 CALL CLFILE (3)
 9090 CALL CLFILE (2)
      RETURN
      END
