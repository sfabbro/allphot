      SUBROUTINE  FUDGE (FILE, NCOL, NROW, MAXCOL, F)
C
C=======================================================================
C
...
C
      ...
      INTEGER MIN0, MAX0
C
      CHARACTER FILE*40, NEWFIL*40, SWITCH*40
      CHARACTER CASE*1
      REAL X, DX, DY, DYSQ, W
      INTEGER I, J, K, L, LBX, LBY, MBX, MBY, IER, NBORD, NPOLY
...
C
      ...
      IF (NEWFIL .EQ. 'END-OF-FILE') RETURN
      CALL COPPIC (NEWFIL, IER)
      IF (IER .NE. 0) RETURN
      LY = 1
      NY = NROW
 ...
 1010 DATA(2)=-1.
      ...
      DO 1100 J=LY,MY
      NY=1
 1100 CALL WRARAY (2, LX, J, NX, NY, MAXCOL, F, IER)
      GO TO 1000
C
C-----------------------------------------------------------------------
C
 5000 CONTINUE
      ...
      NX = MBX-LBX+1
      NY = MBY-LBY+1
      CALL RDARAY (2, LBX, LBY, NX, NY, MAXCOL, F(LBX,LBY), IER)
...
C
      ...
      NX = MX-LX+1
      NY = MY-LY+1
      CALL WRARAY (2, LX, LY, NX, NY, MAXCOL, F(LX,LY), IER)
      GO TO 1000
C
 9000 CONTINUE
      CALL CLPIC (2, IER)
