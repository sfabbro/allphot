...
c=======================================================================c
c
      subroutine  delete  (lun, i)
c
c This function operates on a logical unit number.  The file is assumed 
c to be open.
c
      character name*132
c
      close (lun, status='delete', iostat=i)
      return
      end
c
c=========================================================================
...
c
c Delete a disk file.  The file is assumed to be closed before
c it gets to this point.
c
      character expand*132
      character file*(*)
      open (lun, file=expand(file), status='old', iostat=istat)
      close (lun, status='delete')
c     if (istat .eq. 0) close (lun, status='delete')
      return
      end
c
c====================================================
